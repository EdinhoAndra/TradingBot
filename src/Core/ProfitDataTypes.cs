// Removidos duplicados se já existirem no namespace
...
// Tipos migrados de LegacyProfitTypes.cs
// Removidos duplicados se já existirem no namespace

namespace Edison.Trading.Core
{
    // Tipos ausentes migrados de docs/LegacyProfitTypes.cs
    ...
    public struct TConnectorOffer
    {
        ...
        public TConnectorOffer(double price, long qtd, int agent, long offerId, SystemTime date)
        {
            Price = price;
            Qtd = qtd;
            Agent = agent;
    ...
    // Tipos compartilhados centralizados do Profit/TradingBot
    [StructLayout(LayoutKind.Sequential)]
